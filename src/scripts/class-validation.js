...
function openModal() {
    toggleModal("createClassModal", true);
}

function closeModal() {
    toggleModal("createClassModal", false);
    resetForm("createClassModal", fields);
}

function openJoinModal() {
    toggleModal("joinClassModal", true);
}

function closeJoinModal() {
    toggleModal("joinClassModal", false);
    resetForm("joinClassModal", joinFields);
}
...
["createClassModal", "joinClassModal"].forEach(id => {
    document.getElementById(id).addEventListener("click", function (e) {
        if (e.target === this) {
            id === "createClassModal" ? closeModal() : closeJoinModal();
        }
    });
});
...
const joinFields = {
    invitationCode: {
        input: document.getElementById("invitation_code"),
        error: document.getElementById("error-invitation_code"),
    },
};
...
function validateInvitationCode() {
    const { input, error } = joinFields.invitationCode;
    const value = input.value.trim();

    if (value === "") {
        showError(input, error, "You Forgot to Fill Invitation Code.");
        return false;
    }

    if (value.length !== 6) {
        showError(input, error, "Code must be 6 characters.");
        return false;
    }
...
joinFields.invitationCode.input.addEventListener("blur", validateInvitationCode);
joinFields.invitationCode.input.addEventListener("input", () => {
    if (!joinFields.invitationCode.error.classList.contains("hidden")) {
...
document.querySelector("#joinClassModal form").addEventListener("submit", (e) => {
    if (!validateInvitationCode()) {
        e.preventDefault();
        joinFields.invitationCode.input.focus();
    }
});
...
function resetForm(modalId, fieldGroup) {
    document.querySelector(`#${modalId} form`).reset();
    Object.values(fieldGroup).forEach(({ input, error }) => {
